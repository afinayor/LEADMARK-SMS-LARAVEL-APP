<div class="row">
    <div class="col-sm-7">
        <input class="form-control input-control pull-left" id="search" value="<?php echo e(Session::get('history_search')); ?>"
               onkeydown="if (event.keyCode == 13) ajaxLoad('<?php echo e(url('sms/history/list')); ?>?ok=1&search='+this.value)"
               placeholder="Search..."
               type="text">
        <select  onchange="ajaxLoad('<?php echo e(url('sms/history/list')); ?>?by=1&type='+this.value)" class="form-control slectinput-control" id="contact-group">
            <option <?php echo e((Session::get('history_search_type') == 'subject') ? 'selected' : ''); ?> value="subject">Subject</option>
            <option <?php echo e((Session::get('history_search_type') == 'content') ? 'selected' : ''); ?> value="content">Body</option>
            <option <?php echo e((Session::get('history_search_type') == 'recipients') ? 'selected' : ''); ?> value="recipients">Recipients</option>

        </select>
        <button type="button" class="btn btn-primary btn-search"
                onclick="ajaxLoad('<?php echo e(url('sms/history/list')); ?>?ok=1&search='+$('#search').val())"><i
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <?php foreach($msgs as $msg): ?>
                ...
                <td><a href="javascript:return false" onclick="ajaxLoad('<?php echo e(route('historyDetails',['id'=>$msg->id])); ?>')" ><?php echo e($msg->subject); ?></a></td>
...
<script>
    ...
    function deleteList(values){
        swal({
            ...
            var filename = '<?php echo e(route('postHistDel')); ?>';
...
//                    $('.loading').hide();
    ...
    function resend(id){
        swal({
            ...
            var filename = '<?php echo e(route('resend')); ?>';
            var data = "values="+id;
            $.ajaxSetup({
                headers:{
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
