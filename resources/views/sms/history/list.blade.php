<div class="row">
    <div class="col-sm-7">
        <input class="form-control input-control pull-left" id="search" value="{{ Session::get('history_search') }}"
               onkeydown="if (event.keyCode == 13) ajaxLoad('{{url('sms/history/list')}}?ok=1&search='+this.value)"
               placeholder="Search..."
               type="text">
        <select  onchange="ajaxLoad('{{url('sms/history/list')}}?by=1&type='+this.value)" class="form-control slectinput-control" id="contact-group">
            <option {{(Session::get('history_search_type') == 'subject') ? 'selected' : ''}} value="subject">Subject</option>
            <option {{(Session::get('history_search_type') == 'content') ? 'selected' : ''}} value="content">Body</option>
            <option {{(Session::get('history_search_type') == 'recipients') ? 'selected' : ''}} value="recipients">Recipients</option>

        </select>
        <button type="button" class="btn btn-primary btn-search"
                onclick="ajaxLoad('{{url('sms/history/list')}}?ok=1&search='+$('#search').val())"><i
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            @foreach($msgs as $msg)
                ...
                <td><a href="javascript:return false" onclick="ajaxLoad('{{route('historyDetails',['id'=>$msg->id])}}')" >{{$msg->subject}}</a></td>
...
<script>
    ...
    function deleteList(values){
        swal({
            ...
            var filename = '{{route('postHistDel')}}';
